<?php

namespace Laraflow\Sms;

class Providers
{
    public const AFRICAS_TALKING = 'africastalking';
    public const CLICK_A_TELL = 'clickatell';
    public const CLICK_SEND = 'clicksend';
    public const INFOBIP = 'infobip';
    public const MESSAGE_BIRD = 'messagebird';
    public const SMS_BROADCAST = 'smsbroadcast';
    public const TELNYX = 'telnyx';
    public const TWILIO = 'twilio';

    //Bangladesh
    public const ADN = 'adn';
    public const AJURA_TECH = 'ajuratech';
    public const ALPHA = 'alpha';
    public const BANGLALINK = 'banglalink';
    public const BD_BULK_SMS = 'bdbulksms';
    public const BOOM_CAST = 'boomcast';
    public const BRILLIANT = 'brilliant';
    public const BULK_SMS_BD = 'bulksmsbd';
    public const CUSTOM_GATEWAY = 'customgateway';
    public const DIANA_HOST = 'dianahost';
    public const DIANA_SMS = 'dianasms';
    public const DNS_BD = 'dnsbd';
    public const ELIT_BUZZ = 'elitbuzz';
    public const ESMS = 'esms';
    public const GRAMEENPHONE = 'grameenphone';
    public const GREEN_WEB = 'greenweb';
    public const LPEEK = 'lpeek';
    public const MDL = 'mdl';
    public const METRONET = 'metronet';
    public const MIM_SMS = 'mimsms';
    public const MOBI_REACH = 'mobireach';
    public const MOBI_SHASRA = 'mobishasra';
    public const MUTHOFUN = 'muthofun';
    public const NOVOCOMBD = 'novocombd';
    public const ONNOROKOM = 'onnorokom';
    public const QUICK_SMS = 'quicksms';
    public const REDMO_IT_SMS = 'redmoitsms';
    public const SMART_LAB_SMS = 'smartlabsms';
    public const SMS4_BD = 'sms4bd';
    public const SMS_NET24 = 'smsnet24';
    public const SMS_NOC = 'smsnoc';
    public const SMSINBD = 'smsinbd';
    public const SMSNETBD = 'smsnetbd';
    public const SMSQ = 'smsq';
    public const SSL = 'ssl';
    public const TENSE = 'tense';
    public const TRUBO_SMS = 'trubosms';
    public const TWENTYFOURSMSBD = 'twentyfoursmsbd';
    public const VIATECH = 'viatech';
    public const TWENTY4BULKSMS = 'twenty4bulksms';
}
